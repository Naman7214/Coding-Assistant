...
from MCP.enviornment_tools import run_terminal_command

# from MCP.modification_tools import search_and_replace
from MCP.external_tools import web_search
from MCP.file_access_tools import (
    delete_file,
    list_directory,
    read_file,
    search_files,
)
from MCP.search_tools import codebase_search, execute_grep_search
...
def main(port: int) -> int:
    ...
    async def tool_handler(
        ...
        if name == "grep_search":
            ...
            if "query" and "replacement" and "explanation" not in arguments:
                raise ValueError("Missing required argument 'query' and 'replacement' and 'explanation'")
            result = await search_and_replace(**arguments)
            json_output = json.dumps(result, indent=2)
            return [
                types.TextContent(type="text", text=json_output)
            ]
    ...
    # Run with Uvicorn
    uvicorn.run(starlette_app, host="192.168.17.182", port=port)
