            ...
            if exclude_pattern:
                for root, _, files in os.walk(path):
                    for exc_pattern in exclude_pattern.split(","):
                        ...
                        include_paths = [p for p in include_paths if p not in exclude_paths]
        else:
            include_paths = [path] if self._matches_pattern(path, include_pattern) else []
    ...
    async def _process_file(
        ...
        try:
            ...
            for match in matches:
                ...
                after_ctx = original_content[end:min(len(original_content), end+20)]
            ...
            async with asyncio.Lock():
                ...
                results["error"] = f"Error processing file {file_path}: {str(e)}"
