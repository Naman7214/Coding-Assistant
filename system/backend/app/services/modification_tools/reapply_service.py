...
class ReapplyService:
    ...
    async def reapply_code_changes(
        self, target_file_content: str, code_snippet: str
    ):
        try:
            try:
                edited_content = await self._reapply_code_changes(
                    target_file_content, code_snippet
                )

                if edited_content is None:
                    raise ValueError("Failed to reapply code changes")
                ...
                return {
                    "success": False,
                    "error": f"Reapply model API error: {str(api_error)}",
                    "details": {
    ...
    async def reapply_code_changes_stream(
        self, target_file_content: str, code_snippet: str
        ...
        try:
            ...
            async for event in self._reapply_code_changes_stream(
                target_file_content, code_snippet
            ):
                yield event
    ...
    async def _reapply_code_changes(
        self, original_code: str, code_snippet: str
    ) -> Optional[str]:
        """
        Reapply code changes to the original code using the TGI model.

        Args:
            original_code (str): The original code content
            code_snippet (str): The code snippet to reapply
    ...
    async def _reapply_code_changes_stream(
        self, original_code: str, code_snippet: str
    ) -> AsyncGenerator[str, None]:
        """
