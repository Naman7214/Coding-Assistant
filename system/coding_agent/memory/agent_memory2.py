...
class AgentMemory:
    ...
    def add_assistant_message(self, message: Dict[str, Any]):
        ...
        self.current_context_size += self._estimate_message_size(
            filtered_message
        )
        self._check_and_compress()
                ...
                messages.append(
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": f"Previous context summary: {summary}",
                            }
                        ],
                    }
                )
            ...
            messages.append(
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": f"Current session tools: {tool_summary}",
                        }
                    ],
                }
            )
    ...
    def _get_valid_recent_messages(self) -> List[Dict[str, Any]]:
        ...
        while i < len(recent_messages):
            ...
            if current_msg.get("role") == "assistant" and self._has_tool_use(
                current_msg
            ):
                ...
                if i + 1 < len(recent_messages):
                    ...
                    if next_msg.get("role") == "user" and self._has_tool_result(
                        next_msg
                    ):
                        # Verify the tool_result matches tool_use
                        if self._tool_results_match_tool_uses(
                ...
                valid_messages[-1] = (
                    filtered_msg  # Replace with filtered version
                )
                i += 1

            # If this is a tool_result without preceding tool_use, skip it
            elif current_msg.get("role") == "user" and self._has_tool_result(
                current_msg
            ):
                # Check if previous message has matching tool_use
                if (
                    valid_messages
                    and valid_messages[-1].get("role") == "assistant"
                    and self._has_tool_use(valid_messages[-1])
                    and self._tool_results_match_tool_uses(
                        valid_messages[-1], current_msg
                    )
                ):
                    valid_messages.append(current_msg)
    ...
    def _tool_results_match_tool_uses(
        self, tool_use_msg: Dict[str, Any], tool_result_msg: Dict[str, Any]
    ) -> bool:
    ...
    def _remove_tool_use_from_message(
        ...
        return {"role": message.get("role"), "content": filtered_content}

    def _filter_assistant_message(
        self, message: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _summarize_search_result(self, result: str) -> str:
        ...
        for i, line in enumerate(lines[:20]):  # First 20 lines
            if any(
                ...
                for keyword in ["found", "match", "result"]
            ):
    ...
    def _compress_old_conversations(self):
        ...
        ):  # Keep at least 8 recent messages for tool pairs
            return
    ...
    def _messages_have_valid_tool_pairs(
    ...
    def _create_context_summary(self) -> str:
        ...
        if user_intents:
            summary_parts.append(
                f"Previous requests: {'; '.join(user_intents[-3:])}"
            )
        if assistant_actions:
            summary_parts.append(
                f"Previous actions: {'; '.join(assistant_actions[-3:])}"
            )
        ...
        for i, msg in enumerate(recent_messages):
            ...
            for block in content:
                if block.get("type") == "tool_use":
                    ...
                    tools_info.append(
                        f"tool_result:{block.get('tool_use_id', 'no_id')}"
                    )
