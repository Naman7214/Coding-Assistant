...
class ToolAdapter:
    ...
    async def _fetch_available_tools(self):
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(self._available_tools)} tools from MCP server"
            )
    ...
    async def execute_tool(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            if not tool_info:
                error_msg = (
                    f"Tool '{tool_call.tool_name}' not found in available tools"
                )
                logger.error(error_msg)
                return ToolResult(
                    success=False, content=error_msg, error=error_msg
                )

            # Prepare the request to the MCP server
            endpoint = tool_info.get(
                "endpoint", f"/tools/{tool_call.tool_name}"
            )
            url = f"{self.mcp_base_url}{endpoint}"

            # Execute the tool
            response = await self.client.post(url, json=tool_call.parameters)
            response.raise_for_status()
