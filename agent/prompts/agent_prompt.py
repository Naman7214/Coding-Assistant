...
def create_tool_selection_prompt(agent_state: AgentState) -> Dict[str, Any]:
    ...
    if not agent_state.conversation_history or agent_state.conversation_history[-1]["role"] != "user":
        messages["messages"].append({
            "role": "user",
            "content": [{"type": "text", "text": "Based on our conversation so far, what's the next step? Please select a tool to use or provide a final response."}]
        })
