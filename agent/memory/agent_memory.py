...
import json
import os
from agent.prompts.summery_generation_prompt import SUMMARY_GENERATION_SYSTEM_PROMPT, SUMMARY_GENERATION_USER_PROMPT
...
class MongoDBMemory:
    ...
    def __init__(self):
        ...
        logger.info(f"MongoDB memory initialized with database: {settings.MONGODB_DB_NAME}")
    ...
    async def create_session(self, session_id: Optional[str] = None) -> str:
        ...
        try:
            ...
            logger.info(f"Created new conversation session with ID: {session_id}")
    ...
    async def add_message(self, session_id: str, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if not session_data:
                logger.warning(f"Session not found after adding message: {session_id}")
                return
    ...
    async def _update_sliding_window(self, session_id: str, session_data: Dict[str, Any]) -> None:
        """
        ...
        messages_to_summarize = messages[window_start_index:window_start_index + self.window_size]
        ...
        summary = await self._generate_summary(messages_to_summarize, current_summary)
        ...
        logger.info(f"Updated sliding window for session {session_id}, new window starts at index {new_window_start_index}")
    
    async def _generate_summary(self, messages: List[Dict[str, Any]], previous_summary: Optional[str] = None) -> str:
        """
        ...
        try:
            ...
            system_content = SUMMARY_GENERATION_SYSTEM_PROMPT.format(previous_summary = previous_summary)
            ...
            for msg in messages:
                ...
                if role == "assistant" and "tool_call" in msg:
                    ...
                    conversation_text += f"Assistant (using tool): {tool_info}\n"
                elif role == "tool":
                    tool_result = f"Tool {msg.get('name', 'unknown')}: {content}"
            ...
            formatted_messages.append({
                "role": "user",
                "content": SUMMARY_GENERATION_USER_PROMPT.format(conversation=conversation_text)
            })
            ...
            return previous_summary or "Failed to generate summary due to an error."
    ...
    async def get_full_conversation_history(self, session_id: str) -> List[Dict[str, Any]]:
        """
